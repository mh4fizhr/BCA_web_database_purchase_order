              ...
              <div class="card-header border-0">
                <h3 class="mb-0 text-uppercase d-inline-block"><li class="fa fa-database"></li> &nbspDatabase {{$page2}}</h3>
                  ...
                  <div class="table-responsive">
                    <form action="{{url('backend/admin/report_service/delete')}}" method="post" role="form">
                        {{ csrf_field() }}
                  <table class="table align-items-center table-flush table-hover text-center mydatatable" id="myTable">
                    ...
                    <tbody>
                      <?php 
                          $i = 1;
                        ?>
                        @foreach($report_services as $report_service)
                        @if($report_service->active != '1')
                          <tr role="row" class="odd">
                        @else
                          <tr role="row" class="odd bg-danger text-white">
                        @endif
                              <td>{{$i}}</td>
                              <td>{{$report_service->periode}}</td>
                              <td>{{$report_service->Nopo}}</td>
                              <td>{{$report_service->Sewa}}</td>
                              <td>{{$report_service->CP}}</td>
                              <td>{{$report_service->MerekMobil}} - {{$report_service->Type}}</td>
                              <td>{{$report_service->Nopol}}</td>
                              <td>{{$report_service->NamaVendor}}</td>
                              <td>{{$report_service->KodeCabang}} - {{$report_service->NamaCabang}}</td>
                              <td>{{$report_service->Kota}}</td>
                              <td>{{$report_service->NamaDriver}}</td>
                              <td>{{$report_service->nip}}</td>
                              <td >
                                    @if($report_service->TglService != '')
                                      {{ date('d-M-Y', strtotime($report_service->TglService))}}
                                    @else
                                      
                                    @endif
                              </td>
                              <td >
                                    @if($report_service->km == '')
                                    @else
                                      @currency($report_service->km)
                                    @endif 
                              </td>
                              <td >
                              ...
                              <td>
                                  <div class="custom-control custom-checkbox">
                                    <input type="checkbox" class="custom-control-input" name="report_service[]" id="customCheck{{$i}}" value="{{$report_service->id}}">
