          ...
          <div class="col-12">
            <div class="card pb-4">
              <div class="card-header border-0">
                <h3 class="mb-0 text-uppercase d-inline-block"><li class="fas fa-file-word nav-icon"></li> &nbspDatabase Surat perubahan</h3>
                <!-- <div class="dropdown float-right">
                  ...
                  <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                    <a class="dropdown-item" href="{{url('/backend/surat/perubahan/status/active')}}">not approved</a>
                    <a class="dropdown-item" href="{{url('/backend/surat/perubahan/status/deactive')}}">approved</a>
                  </div>
                </div> -->
              </div>
              
              @if($s == 'active')
                 <form action="{{url('/backend/surat/perubahan/status')}}" method="post" role="form">
                    {{ csrf_field() }}
                  <div class="table-responsive">
                    ...
                    <table class="table align-items-center table-flush table-hover text-center mydatatable" id="myTable">
                      ...
                      <tbody>
                        ...
                        <tr>
                          <td>{{$i}}</td>
                          <td>{{$template_perubahan->nama_vendor}}</td>
                          <td>
                            {{$template_perubahan->no_surat}}
                            @if($currentDateTime == date('d-m-Y', strtotime($template_perubahan->created_at)))
                               <sup class=""><span class="badge badge-warning">new</span></sup>
                            @endif
                          </td>
                          <td>{{$template_perubahan->tgl_surat}}</td>
                          <td>
                            @if($template_perubahan->status == '1' || $template_perubahan->status == '2')
                            <a href="{{url('/backend/surat/perubahan/'.$template_perubahan->id)}}" class="btn btn-success btn-sm"><i class="fas fa-download"></i> &nbspdownload</a>
                            @endif
                            <a href="{{url('/backend/surat/perubahan/view/'.$template_perubahan->id)}}" class="btn btn-info btn-sm"><i class="fas fa-file-alt"></i> &nbspview</a>
                          </td>
                          <td>
                          ...
                          <!-- <td>
                            ...
                            @else
                              <div class="custom-control custom-checkbox">
                                <input type="checkbox" class="custom-control-input" name="status[]" id="customCheck{{$i}}" value="{{$template_perubahan->id}}">
             ...
             @endif
                <!-- <div class="table-responsive">
                    ...
                    <form action="{{url('/backend/surat/perubahan/delete_multiple')}}" method="post" role="form">
                    {{ csrf_field() }}
                    <table class="table align-items-center table-flush table-hover text-center mydatatable" id="myTable">
                      <thead class="thead-light" style="height: 70px">
                        <tr>
