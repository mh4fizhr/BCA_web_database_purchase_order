          ...
          <div class="col-12">
            <div class="card pb-4">
              <div class="card-header border-0">
                <h3 class="mb-0 text-uppercase d-inline-block"><li class="fas fa-award nav-icon"></li> &nbspDatabase Jabatan</h3>
                <div class="dropdown float-right">
                  ...
                  <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                    <a class="dropdown-item" href="{{url('/backend/pejabat/jabatan/active')}}">Active</a>
                    <a class="dropdown-item" href="{{url('/backend/pejabat/jabatan/deactive')}}">Deactive</a>
                  </div>
                </div>
              </div>
              
              @if($s == 'active')
                 <form action="{{url('/backend/pejabat/jabatan/delete_multiple')}}" method="post" role="form">
                    {{ csrf_field() }}
                  <div class="">
                    <!-- Projects table -->
...
<!--                                   <a class="btn btn-primary btn-sm" href="#">
                                  ...
                                  <a class="btn btn-success btn-sm" href="{{url('/backend/pejabat/jabatan/edit/'.$jabatan->id)}}">
                              ...
                              <td>
                                  <div class="custom-control custom-checkbox">
                                    <input type="checkbox" class="custom-control-input" id="customCheck{{$i}}" value="{{$jabatan->id}}">
                    ...
                    <?php $i = 1; ?>
                    @foreach(App\jabatan::all()->sortBy('id') as $jabatan)
                        @if($jabatan->active != '1')
                          <div class="delete_checkbox{{$jabatan->id}}"></div> 
                          <?php $i = $jabatan->id; ?>                    
                        @endif
                    @endforeach
                  </div>
...
$(document).ready(function(){
  ...
  $("#jabatan").keyup(function(){
    ...
    $.ajax({
        url:"{{ route('jabatan_available.check') }}",
        method:"POST",
        data:{jabatan:jabatan, _token:_token},
        success:function(result)
        {
         if(result == 'unique')
         {
            $('#error_jabatan').html('<label class="text-success">*jabatan belum tersedia</label>');
            $('#jabatan').removeClass('has-error');
            $('#submit').attr('disabled', false);
         }
         else
         {
            $('#error_jabatan').html('<label class="text-danger">*jabatan sudah ada</label>');
