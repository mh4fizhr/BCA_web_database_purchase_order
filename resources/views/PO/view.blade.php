...
<?php
    ...
    if (App\approve::where('po_id', $po->id)->where('approve', 'waiting bop')->exists()) {
        $status_approve = 'waiting bop';
    }elseif (App\approve::where('po_id', $po->id)->where('approve', 'waiting2')->exists()) {
        $status_approve = 'waiting2';
    }elseif (App\approve::where('po_id', $po->id)->where('approve', 'waiting')->exists()) {
                  ...
                  <div class="col-md-8">
                    <div class="row">
                        ...
                        <div class="form-group">
                          <span>:
                            @if($status_approve == 'waiting bop' || $status_approve == 'waiting2')
                              <span class="h4 font-weight-bold mb-0">Mobil</span>
                        ...
                        <div class="form-group">: 
                            ...
                            @foreach($mobils as $mobil)
                              @if($po->Mobil_id == $mobil->id)
                                ...
                                &nbsp
                                @if(auth::user()->status == 'operasional' || auth::user()->status == 'pengada')
                      ...
                      <div class="col-md-9 mt--1">
                        <div class="form-group">
                          <span>: 
                            @if($po->Sewa_sementara == "null" && $approve_pengurangan != 'waiting')
                            ...
                            @else
                              ...
                              @else
                                <a href="{{url('/backend/po/relokasi/tampungan/'.$po->id)}}" class="btn btn-sm btn-icon btn-info">
                              ...
                              @if($approve_pengurangan == 'waiting')
                              ...
                              @else
                                ...
                                <a class="btn btn-danger btn-icon btn-sm" href="{{url('/backend/po/pengurangan_driver/tampungan/'.$po->id)}}">
                                    <span class="btn-inner--icon"><i class="fas fa-file-download"></i></span>
                                    <span class="btn-inner--text">Pengurangan</span>
                                </a>
                                @else
                                <a class="btn btn-danger btn-icon btn-sm" href="{{url('/backend/po/pengurangan/tampungan/'.$po->id)}}">
                                    <span class="btn-inner--icon"><i class="fas fa-file-download"></i></span>
                                    <span class="btn-inner--text">Pengurangan</span>
                                </a>
                                @endif
                              ...
                              @else
                                ...
                                <a class="btn btn-warning btn-icon btn-sm" href="{{url('/backend/po/perubahan/tampungan/'.$po->id)}}">
                                    <span class="btn-inner--icon"><i class="fas fa-window-restore"></i></span>
                                    <span class="btn-inner--text">Perubahan</span>
                                </a>
                                @else
                                <a class="btn btn-warning btn-icon btn-sm disabled" href="{{url('/backend/po/perubahan/tampungan/'.$po->id)}}">
                                    <span class="btn-inner--icon"><i class="fas fa-window-restore"></i></span>
                                    <span class="btn-inner--text">Perubahan</span>
                                </a>
                                @endif
                    ...
                    <div class="row">
                      <!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
                        ...
                        <div class="col-md-6">
                          <div class="form-group">
                            <span>: 
                              @if($po->Efisien_relokasi <= $currentDateTime && $po->Efisien_relokasi != ''  && $status_approve == 'null')
                            ...
                            <span>: 
                              @foreach($vendors as $vendor)
                                @if($po->Vendor_Driver == $vendor->id)
                                  <span class="h5 font-weight-bold mb-0" data-toggle="tooltip" data-placement="top" title="Click to detail"><a href="#" style="color: black" data-toggle="modal" data-target="#vendor">{{$vendor->KodeVendor}}</a></span>
                                  <?php $vendor_ump = $vendor->KodeVendor; ?>
                                @endif
                              @endforeach
                            </span>
                          </div>
                        ...
                        <div class="col-md-6">
                          <div class="form-group">
                            ...
                            ?>
                            : 
                            @if($status_approve == 'null' && ($po->Efisien_relokasi <= $currentDateTime && $po->Efisien_relokasi != '') || ($po->Tgl_cutoff <= $currentDateTime && $po->Tgl_cutoff != ''))
                              @if($status_approve == 'null' && ($po->Sewa_sementara == 'null' || $po->SelesaiSewa < $currentDateTime))
                                <span class="badge badge-sm badge-danger">Not Active</span>
                              @else
                                <span class="badge badge-sm badge-success">Active</span>
                              @endif
                            @elseif($status_approve == 'null' && $po->SelesaiSewa < $currentDateTime)
                              <span class="badge badge-sm badge-danger">Not Active</span>
                            @else
                              <span class="badge badge-sm badge-success">Active</span>
                            @endif
                        ...
                        <div class="col-md-12">
                          <div class="form-group">
                            @if($check_timeline == 'no')
                              <a href="#" class="btn btn-icon btn-default ml-3">
                    ...
                    <tbody>
                        <tr role="row" class="odd">
                          ...
                          <td><!-- <a href="#" class="po_tgl" 
                            ...
                            @if($po->Tgl_cutoff != '' && $po->Sewa_sementara == 'null')
                          ...
                          @if(auth::user()->status == 'operasional' || auth::user()->status == 'pengada')
                          <td>
                            <a class="btn btn-success btn-sm" href="{{url('/backend/po/tgl/edit/'.$po->id)}}">
                                <i class="fas fa-pencil-alt">
                                </i> &nbspEdit
                            </a>
                            <a class="btn btn-warning btn-sm" href="{{url('/backend/po/perpanjang/'.$po->id)}}">
                        ...
                        @if($po->MulaiSewa2 != '' && $po->SelesaiSewa != '' && $po->status == '1')
                        <tr role="row" class="odd">
                          <td><!-- <a href="#" class="po_tgl" 
                            data-name="mulaisewa" 
                            data-type="date" 
                  ...
                  <div class="col-5">
                    <div class="table-responsive">
                      <table class="table align-items-center table-flush text-left " id="myTable">
                        ...
                        <tbody>
                            <tr role="row" class="odd">
                              <td>Harga Sewa Mobil (include)- PPN 10%</td>
                            ...
                            <tr style="background-color: yellow">
                              <td>Harga Sewa Mobil + Driver (include) - PPN 10%</td>
                              <td>
                        ...
                        <tbody>
                            <tr role="row" class="odd">                              
                                ...
                                <td>
                                  ...
                                  <a href="{{url('/backend/po/service/'.$po->id)}}" class="btn btn-sm btn-dark <?php if(auth::user()->status != 'operasional') echo "disabled" ?>"><i class="ni ni-settings"></i>&nbsp Detail Service</a>
          
                                </td>

                                <td>
                                  
                                  <a href="{{url('/backend/po/salon/'.$po->id)}}" class="btn btn-sm btn-dark <?php if(auth::user()->status != 'operasional') echo "disabled" ?>"><i class="ni ni-settings"></i>&nbsp Detail Salon </a>
                                  
                                </td>

                                <td>
                                  
                                    <a href="{{url('/backend/po/mcu/'.$po->id)}}" class="btn btn-sm btn-dark <?php if(auth::user()->status != 'operasional') echo "disabled" ?>"><i class="ni ni-settings"></i>&nbsp Detail MCU & Seragam</a>
